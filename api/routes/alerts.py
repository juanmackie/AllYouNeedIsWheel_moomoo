    ...
    try:
        ...
        for pos in option_positions:
            ...
            if decision.roll_pressure >= 70:
                alerts.append({
                    'ticker': decision.ticker, 'option_type': decision.option_type,
                    'strike': decision.strike, 'expiration': decision.expiration,
                    'alert_type': 'roll_pressure_urgent', 'severity': 'urgent',
                ...
                alerts.append({
                    'ticker': decision.ticker, 'option_type': decision.option_type,
                    'strike': decision.strike, 'expiration': decision.expiration,
                    'alert_type': 'roll_pressure_watch', 'severity': 'warning',
                    'message': (
                        f'Moderate roll pressure ({decision.roll_pressure:.0f}%)'
                    ),
                    'wheel_decision': decision.to_dict(),
                })
            ...
            if decision.profit_target_progress >= 50:
                alerts.append({
                    'ticker': decision.ticker, 'option_type': decision.option_type,
                    'strike': decision.strike, 'expiration': decision.expiration,
                    'alert_type': 'profit_target_50', 'severity': 'info',
                    'message': (
                        f'50% profit target reached ({decision.profit_target_progress:.0f}%)'
                    ),
                    'wheel_decision': decision.to_dict(),
                })
            ...
            if decision.otm_pct < 0:
                alerts.append({
                    'ticker': decision.ticker, 'option_type': decision.option_type,
                    'strike': decision.strike, 'expiration': decision.expiration,
                    'alert_type': 'strike_crossed', 'severity': 'danger',
            ...
            for blocker in decision.hard_blockers:
                alerts.append({
                    'ticker': decision.ticker, 'option_type': decision.option_type,
                    'strike': decision.strike, 'expiration': decision.expiration,
                    'alert_type': 'hard_blocker', 'severity': 'danger',
