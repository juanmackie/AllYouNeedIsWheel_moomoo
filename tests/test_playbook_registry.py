...
class TestPlaybookRoutesMissingIDs(unittest.TestCase):
    """Route-level tests for missing hypothesis IDs returning 404."""

    def setUp(self):
        from flask import Flask
        from api.routes.playbook import bp
    ...
    @patch("api.routes.playbook._get_registry")
    def test_update_status_missing_id_returns_404(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.update_status.return_value = False
        mock_get_reg.return_value = mock_reg

        resp = self.client.put(
            '/api/playbook/hypotheses/nonexistent/status',
            json={"status": "validated"},
        )
        self.assertEqual(resp.status_code, 404)

    @patch("api.routes.playbook._get_registry")
    def test_update_notes_missing_id_returns_404(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.update_notes.return_value = False
        mock_get_reg.return_value = mock_reg

        resp = self.client.put(
            '/api/playbook/hypotheses/nonexistent/notes',
            json={"notes": "updated"},
        )
        self.assertEqual(resp.status_code, 404)

    @patch("api.routes.playbook._get_registry")
    def test_delete_missing_id_returns_404(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.delete.return_value = False
        mock_get_reg.return_value = mock_reg

        resp = self.client.delete('/api/playbook/hypotheses/nonexistent')
        self.assertEqual(resp.status_code, 404)

    @patch("api.routes.playbook._get_registry")
    def test_update_status_invalid_returns_400(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_get_reg.return_value = mock_reg

        resp = self.client.put(
            '/api/playbook/hypotheses/test_hyp/status',
            json={"status": "not_a_status"},
        )
        self.assertEqual(resp.status_code, 400)


class TestPlaybookRoutesConnectionCleanup(unittest.TestCase):
    """Route-level tests verifying registry connections are always closed."""
    ...
    @patch("api.routes.playbook._get_registry")
    def test_list_hypotheses_closes_registry(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.list_all.return_value = []
        mock_get_reg.return_value = mock_reg

        resp = self.client.get('/api/playbook/hypotheses')
        self.assertEqual(resp.status_code, 200)
        mock_reg.close.assert_called_once()

    @patch("api.routes.playbook._get_registry")
    def test_get_hypothesis_closes_registry(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.get.return_value = {"hypothesis_id": "test"}
        mock_get_reg.return_value = mock_reg

        resp = self.client.get('/api/playbook/hypotheses/test')
        self.assertEqual(resp.status_code, 200)
        mock_reg.close.assert_called_once()

    @patch("api.routes.playbook._get_registry")
    def test_create_hypothesis_closes_registry(self, mock_get_reg):
        mock_reg = MagicMock()
        mock_reg.create.return_value = True
        mock_get_reg.return_value = mock_reg

        resp = self.client.post(
            '/api/playbook/hypotheses',
            json={"hypothesis_id": "new_hyp", "title": "New", "description": "Desc"},
        )
