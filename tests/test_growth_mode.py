...
class TestGrowthModeScoringIntegration(unittest.TestCase):
    ...
    def test_growth_mode_ranks_conservative_above_aggressive_when_safer(self):
        ...
        opt_b = _make_option(strike=90, bid=1.20, ask=1.35, oi=900, vol=500, delta=-0.15, iv=0.25, option_type="PUT")
    ...
    def test_high_premium_blocked_when_exceeds_drawdown_budget(self):
        ...
        opt = _make_option(strike=70, bid=8.0, ask=8.50, oi=400, vol=200, delta=-0.45, iv=0.50, option_type="PUT")
    ...
    def test_low_premium_covered_call_on_strong_holding_blocked(self):
        ...
        """
        future = (datetime.now() + timedelta(days=21)).strftime("%Y%m%d")
        opt = _make_option(strike=101, bid=0.30, ask=0.40, oi=500, vol=200, delta=0.10, iv=0.20, option_type="CALL")
        ...
        self.assertEqual(
            result.covered_call_intent,
            "upside-capping risk",
            f"Expected upside-capping risk, got: {result.covered_call_intent}",
        )
        self.assertFalse(
            result.hard_blockers, f"Expected low-premium CC to warn, not hard-block, got: {result.hard_blockers}"
        )
    ...
    def test_yfinance_fallback_lowers_confidence(self):
        ...
        self.assertIsNotNone(result)
        self.assertLess(result.confidence_score, 90, "yfinance fallback should reduce confidence")
...
class TestGrowthModeCSPProfile(unittest.TestCase):
    ...
    def test_higher_delta_csp_outranks_conservative_when_fits_cash(self):
        ...
        opt_a = _make_option(strike=92, bid=4.50, ask=4.80, oi=600, vol=300, delta=-0.30, iv=0.40, option_type="PUT")
        ...
        opt_b = _make_option(strike=90, bid=1.20, ask=1.35, oi=900, vol=500, delta=-0.22, iv=0.25, option_type="PUT")
    ...
    def test_unaffordable_csp_skipped_with_visible_blocker(self):
        ...
        opt = _make_option(strike=950, bid=20.0, ask=21.0, oi=400, vol=200, delta=-0.25, iv=0.35, option_type="PUT")
        small_portfolio = dict(
            self.portfolio, cash_available_for_csp=10000.0, cash_balance=10000.0, account_value=20000.0
        )
        ...
        self.assertIsNotNone(result)
        self.assertTrue(
            result.hard_blockers, f"Expected hard_blockers for unaffordable CSP, got: {result.hard_blockers}"
        )
        blocker_text = " ".join(result.hard_blockers).lower()
        self.assertIn(
            "insufficient cash", blocker_text, f"Expected 'insufficient cash' in blockers: {result.hard_blockers}"
        )
