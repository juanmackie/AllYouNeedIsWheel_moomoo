...
class TestTopRecommendations(unittest.TestCase):
    ...
    def test_include_long_options_cache_key_and_generation_arg(self, mock_cache, mock_probe, mock_get_svc, mock_gen):
        ...
        app = _make_app()
        with app.test_client() as client:
            resp = client.get("/api/options/top-recommendations", query_string={"include_long_options": "true"})
            data = resp.get_json()
        ...
        self.assertTrue(
            args[0].startswith(
                "top_recommendations:limit=3:include_long_options=True:ignore_cash_limits=False:screener=none:hash="
            )
        )
    ...
    def test_ignore_cash_limits_cache_key_and_generation_arg(self, mock_cache, mock_probe, mock_get_svc, mock_gen):
        ...
        app = _make_app()
        with app.test_client() as client:
            resp = client.get("/api/options/top-recommendations", query_string={"ignore_cash_limits": "true"})
            data = resp.get_json()
        ...
        self.assertTrue(
            args[0].startswith(
                "top_recommendations:limit=3:include_long_options=True:ignore_cash_limits=True:screener=none:hash="
            )
        )
        self.assertEqual(kwargs["include_long_options"], True)
