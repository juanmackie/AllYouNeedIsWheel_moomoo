...
class TestWheelAdvisorEvidence(unittest.TestCase):
    ...
    def test_to_prompt_unknown_for_missing_evidence(self):
        ...
        evidence.portfolio = EvidenceBlock(
            category="portfolio", label="Portfolio", available=False,
        )
        prompt = evidence.to_prompt()
    ...
    def test_evidence_unknown_when_block_missing_content(self):
        ...
        evidence.earnings = EvidenceBlock(
            category="earnings", label="Earnings Calendar",
...
class TestBuildEvidenceFromContext(unittest.TestCase):
    ...
    def test_scored_positions_override_positions(self):
        context = {
            ...
            "scored_positions": [
                {
                    "ticker": "AAPL", "option_type": "PUT", "strike": 145.0,
                    "expiration": "20261220", "dte": 21,
                    "stock_price": 150.0, "mid_price": 2.5,
                    "otm_pct": 3.3, "roll_pressure": 30.0,
    ...
    def test_opportunities_evidence(self):
        context = {
            "opportunities": [
                {
                    "ticker": "AAPL", "option_type": "PUT", "strike": 145.0,
                    "expiration": "20261220", "premium_per_contract": 250.0,
                    "delta": -0.20, "dte": 21, "annualized_return": 15.5,
                    "score": 85.0,
                },
            ],
        }
    ...
    def test_evidence_missing_all_sections_unknown(self):
        ...
        for block in evidence.all_blocks():
            self.assertFalse(
                "infer" in block.content.lower() or "guess" in block.content.lower()
            )
    ...
    def test_evidence_opportunities_limited_to_five(self):
        many = [
            {
                "ticker": f"T{i}", "option_type": "PUT", "strike": 100.0,
                "expiration": "20261220", "premium_per_contract": 100.0,
                "delta": -0.20, "dte": 21, "annualized_return": 10.0,
                "score": float(90 - i),
            }
            for i in range(10)
        ]
    ...
    def test_no_blank_evidence_lines_in_prompt(self):
        ...
        for line in evidence_section.split("\n"):
            if not line.strip() or line.startswith("Generated") or line.startswith("IMPORTANT") or line.startswith("Do not") or line.startswith("---") or line.startswith("Based"):
                continue
            if line.startswith("["):
