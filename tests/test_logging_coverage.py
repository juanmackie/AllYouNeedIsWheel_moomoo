...
class TestWheelDecisionLogging(unittest.TestCase):
    ...
    def test_score_contract_success_logs_info(self):
        ...
        profile = {
            'ideal_spread_pct': 12, 'ideal_open_interest': 500,
            'ideal_volume': 1000, 'target_iv_adjusted': 50,
            'target_delta': 0.30, 'delta_tolerance': 0.12,
            'preferred_dte': 37, 'target_capital_efficiency': 100,
            'target_theta_delta_ratio': 0.005,
            'min_mid_price': 0.05, 'max_spread_pct': 60,
            'min_premium_per_contract': 10,
            'min_open_interest': 10, 'min_volume': 1,
            'default_otm_pct': 10, 'profile_type': 'monthly',
        }
        portfolio = {
            'positions': {}, 'cash_balance': 50000,
            'available_cash': 50000, 'cash_available_for_csp': 50000,
            'account_value': 100000, 'vix_regime': {'vix': 15, 'regime': 'normal'},
        }
        option = {
            'strike': 95.0, 'expiration': '20260718',
            'option_type': 'PUT', 'bid': 1.50, 'ask': 1.60,
            'delta': -0.30, 'gamma': 0.02, 'theta': -0.05, 'vega': 0.10,
            'implied_volatility': 0.25, 'open_interest': 500, 'volume': 200,
        }
        with self.assertLogs('core.wheel_decision', level='INFO') as log:
            result = score_contract(
                'TEST', option, 100.0, profile, portfolio,
                iv_status_str='normal', iv_rank=0.5,
            )
...
class TestGreeksLogging(unittest.TestCase):
    ...
    def test_enrich_option_with_greeks_logs_info(self):
        ...
        option = {
            'strike': 100.0, 'expiration': '20260918',
            'option_type': 'CALL', 'implied_volatility': 0.30,
...
class TestDatabaseLogging(unittest.TestCase):
    ...
    def test_init_logs_info(self, mock_reg, mock_migrate, mock_create, mock_pool):
        ...
        with patch('db.database.IVRepository'), \
            ...
            with self.assertLogs('db.database', level='INFO') as log:
                ...
                db = OptionsDatabase(db_name=':memory:')
...
class TestApiLogging(unittest.TestCase):
    ...
    def test_health_request_logs_completion(self):
        ...
        with patch('api.get_service', return_value=mock_tvscreener), \
    ...
    def test_unauthorized_request_logs_warning(self):
        ...
        self.assertTrue(any('Unauthorized API request method=GET path=/portfolio request_id=req-unauth' in msg for msg in log.output))
    ...
    def test_health_probe_failures_log_warnings(self):
        ...
        with patch('api.get_service', side_effect=RuntimeError('tvscreener down')), \
             patch('api.sqlite3.connect', return_value=mock_conn), \
             patch('api.probe_opend_status', side_effect=RuntimeError('opend down')):
            app = self._build_health_app(config={'database': database, 'connection_config': {'host': '127.0.0.1', 'port': 11111}})
