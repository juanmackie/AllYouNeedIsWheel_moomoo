...
export async function fetchOptionData(ticker, otmPercentage = 10, optionType = null, expiration = null) {
    try {
        ...
        const response = await fetch(url, { headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache', 'Expires': '0' } });
        if (!response.ok) {
        ...
        if (!isRealAccountUnavailableError(error)) showAlert(`Error fetching options for ${ticker}: ${error.message}`, 'danger');
...
export async function fetchTopRecommendations(limit = 3, manualRefresh = false) {
    try {
        ...
        if (!response.ok) {
            const payload = await readJsonSafely(response);
            if (isOpenDUnavailable(payload)) { setConnectionStatusFromPayload(payload); return { recommendations: [], count: 0, error: payload?.error || 'OpenD unavailable' }; }
            throw new Error(payload?.error || `HTTP error ${response.status}`);
        }
        clearUnavailableStatus();
        const result = await response.json();
