...
/**
 * Create a sparkline from price data
 * @param {string} ticker - Stock symbol
 * @param {HTMLElement} container - Container element
 * @param {number} days - Number of days (default 24 for hourly, 30 for daily)
 */
export async function createPriceSparkline(ticker, container, days = 24) {
    try {
        const response = await fetch(`/api/options/stock-price?tickers=${ticker}`);
        ...
        createSparkline(container, prices.slice(-days), {
            width: 100,
            height: 30,
            color: 'auto',
            strokeWidth: 1.5,
...
/**
 * Clear sparklines from a container
 * @param {HTMLElement} container
 */
export function clearSparklines(container) {
    if (!container) return;
